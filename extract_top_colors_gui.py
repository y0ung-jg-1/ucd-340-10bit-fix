...
class ColorExtractor:
    ...
    def batch_extract(self, input_dir, output_csv, enable_dedup=True,
                      bit_depth=10, progress_callback=None, stop_flag=None):
        """批量提取颜色并输出CSV"""
        ...
        for i, bin_file in enumerate(bin_files):
            ...
            if enable_dedup and color == prev_color:
                skipped += 1
...
class Application(tk.Tk):
    ...
    def _setup_layout(self):
        ...
        ttk.Checkbutton(param_frame, text="启用去重(跳过相同颜色)", variable=self.dedup_var).grid(row=2, column=0, columnspan=4, sticky=tk.W, pady=(5,0))
    ...
    def _process_task(self):
        ...
        valid, skipped, status = extractor.batch_extract(
            bin_dir, csv_path, enable_dedup, bit_depth, progress_callback, stop_check
        )
